...
export default function ViewerPage() {
  ...
  //   isConnected,
  //   timerIdFromUrl,
  //   timerIdFromUrlType: typeof timerIdFromUrl,
  //   timerList: timerList.length,
  //   timerListIds: timerList.map(t => t.id),
  //   currentTimer,
  //   selectedTimerId
  // });
  ...
  useEffect(() => {
    if (timerIdFromUrl && isConnected) {
      // Timer IDs are now strings, not integers
      // console.log('Joining timer from URL:', timerIdFromUrl);
      // console.log('Available timers:', timerList.map(t => ({ id: t.id, name: t.name })));
      joinTimer(timerIdFromUrl);
    } else if (!timerIdFromUrl && isConnected && timerList.length > 0) {
      // console.log('No timer in URL, showing timer selection');
    } else if (!timerIdFromUrl && isConnected && timerList.length === 0) {
      // console.log('No timer in URL and no timers available');
    }
